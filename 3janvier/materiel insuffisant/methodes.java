...
public class methodes {
    ...
    public static int coordonneeColonne() {
        ...
        int colonne = conversionColEnInt(col);
    ...
    public static boolean mouvementRoiRobot(int ligne, int col, int nvL, int nvC) {
        int distL = Math.abs(nvL - ligne);
        int distC = Math.abs(nvC - col);

        return (distL <= 1 && distC <= 1 && (distL != 0 || distC != 0));
    }
    ...
    public static void destinationPiece(int[][] plateau, int ligne, int colonne, int piece, char joueur) {
        ...
        do {
            ...
            if (piece == 1 || piece == 7 || piece == 4 || piece == 11) {
                if (mouvementTour(ligne, colonne, NvLigne, NvColonne)){
                    mouvementValide = true;
                }
            }
            if (piece == 3 || piece == 9 || piece == 4 || piece == 11) {
                if (mouvementFou(ligne, colonne, NvLigne, NvColonne)) {
                    mouvementValide = true;
                }
            }
            if (piece == 2 || piece == 8) {
                ...
                if (piece != 0 && ((joueur == 'B' && piece > 6) || (joueur == 'J' && piece <= 6))) {
                    ...
                    for (int nl = 0; nl < 8; nl++) {
                        for (int nc = 0; nc < 8; nc++) {
                            ...
                            boolean mouvementPossible = peutAttaquer(plateau, l, c, nl, nc) ||
                                    ((piece == 6 || piece == 12) && mouvementPionRobot(plateau, l, c, nl, nc, piece));

                            if (mouvementPossible) {
                                // On vérifie la destination
                                if (!memeCouleur(plateau, nl, nc, piece)) {

                                    // Si la destination est libre on simule le coup sur une copie du plateau

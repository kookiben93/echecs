...
public class methodes {
    ...
    //fonction qui retourne vrai si le mouvement du Cavalier est en L est impossible selon sa position (mode 2)
    public static boolean bougerCavalierPossible(int[][] plateau, int ligne, int colonne) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (caseValide(nouvelleL, nouvelleC)) {     //vérifie que la combinaison est sur le plateau pour éviter les bugs de sorties de plateau
                ...
                if (plateau[nouvelleL][nouvelleC] == 0 || pieceEnFace != bleu) {       //si la case est vide ou si elle est occupée par une pièce ennemie
    ...
    //fonction qui retourne vrai si les cases+1 du haut, du bas, de gauche et de droite sont indisponibles pour la pièce jouée actuelle
    public static boolean pieceAutour(int[][] plateau, int ligne, int colonne) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (caseValide(nouvelleL, nouvelleC) && !metEnEchec(plateau, ligne, colonne, nouvelleL, nouvelleC, piece)){  //si la case existe sur le plateau et le déplacement ne met pas en échec le roi
    ...
    public static boolean metEnEchec(int[][] plateau, int ligne, int colonne, int nouvelleL, int nouvelleC, int piece){
        int[][] u = copiePlateau(plateau);
        char joueur;
        ...
        u[ligne][colonne] = 0;
        u[nouvelleL][nouvelleC] = piece;

        return estEnEchec(u, joueur);
    }

    //fonction qui retourne vrai si les cases+1 des 4 diagonales sont indisponibles pour la pièce jouée actuelle
    ...
    //fonction qui renvoie vrai si le pion ne peut pas bouger, vrai sinon
    public static boolean pieceAutour3(int[][] plateau, int ligne, int colonne) {
        ...
        //si une des options est possible sans mettre en échec le roi, on renvoie faux
        if ((diagoGauche || diagoDroite || peutAvancer) && !metEnEchec(plateau, ligne, colonne, ligne+sens, colonne, pion)) {
            return false;
        } else {
            return true;
        }
